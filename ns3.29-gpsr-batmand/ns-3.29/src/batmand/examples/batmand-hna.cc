...
//
// One way is to use batmand::RoutingProtocol::SetRoutingTableAssociation ()
// to use which you may run:
//
// ./waf --run "batmand-hna --assocMethod1=1"
//
// The other way is to use batmand::RoutingProtocol::AddHostNetworkAssociation ()
// to use which you may run:
//
// ./waf --run "batmand-hna --assocMethod2=1"
//
...
NS_LOG_COMPONENT_DEFINE ("BatmandHna");
...
int main (int argc, char *argv[])
{
  ...
  cmd.AddValue ("aggregate", "Allows use of OGM Aggregation", aggregation);
  ...
  //Config::SetDefault ("ns3::Batmand::Aggregation", aggregation );

    
  NodeContainer batmandNodes;
  batmandNodes.Create (2);
  ...
  NetDeviceContainer devices = wifi.Install (wifiPhy, wifiMac, batmandNodes);

  CsmaHelper csma;
  csma.SetChannelAttribute ("DataRate", DataRateValue (DataRate (5000000)));
  csma.SetChannelAttribute ("Delay", TimeValue (MilliSeconds (2)));
  NetDeviceContainer csmaDevices = csma.Install (NodeContainer (csmaNodes.Get (0), batmandNodes.Get (1)));
  ...
  mobility.Install (batmandNodes);

  BatmandHelper batmand;
  OlsrHelper olsr;

  // Specify Node B's csma device as a non-BATMAND device.
  batmand.ExcludeInterface (batmandNodes.Get (1), 2);
  ...
  InternetStackHelper internet_batmand;
  internet_batmand.SetRoutingHelper (list); // has effect on the next Install ()
  internet_batmand.Install (batmandNodes);
  ...
  Ptr<Socket> source = Socket::CreateSocket (batmandNodes.Get (0), tid);
  InetSocketAddress remote = InetSocketAddress (Ipv4Address ("172.16.1.1"), 80);
  source->Connect (remote);

  // Obtain batmand::RoutingProtocol instance of gateway node
  // (namely, node B) and add the required association
  Ptr<Ipv4> stack = batmandNodes.Get (1)->GetObject<Ipv4> ();
  Ptr<Ipv4RoutingProtocol> rp_Gw = (stack->GetRoutingProtocol ());
  Ptr<Ipv4ListRouting> lrp_Gw = DynamicCast<Ipv4ListRouting> (rp_Gw);

  Ptr<batmand::RoutingProtocol> batmandrp_Gw;

  for (uint32_t i = 0; i < lrp_Gw->GetNRoutingProtocols ();  i++)
    {
      int16_t priority;
      Ptr<Ipv4RoutingProtocol> temp = lrp_Gw->GetRoutingProtocol (i, priority);
      if (DynamicCast<batmand::RoutingProtocol> (temp))
        {
          batmandrp_Gw = DynamicCast<batmand::RoutingProtocol> (temp);
        }
    }
  ...
  if (assocMethod1)
    {
      ...
      // which are associated with non-BATMAND interfaces specified above.
      hnaEntries->AddNetworkRouteTo (Ipv4Address ("172.16.1.0"), Ipv4Mask ("255.255.255.0"), uint32_t (2), uint32_t (1));
      batmandrp_Gw->SetRoutingTableAssociation (hnaEntries);
    }

  if (assocMethod2)
    {
      // Specify the required associations directly.
      batmandrp_Gw->AddHostNetworkAssociation (Ipv4Address ("172.16.1.0"), Ipv4Mask ("255.255.255.0"));
    }

  // Tracing
  wifiPhy.EnablePcap ("batmand-hna", devices);
  csma.EnablePcap ("batmand-hna", csmaDevices, false);
